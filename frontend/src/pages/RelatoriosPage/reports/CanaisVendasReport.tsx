...
const CanaisVendasReport: React.FC = () => {
  ...
  const load = async () => {
    setLoading(true);
    try { setData(await reportService.salesChannelsReport({ date_from: dateFrom, date_to: dateTo })); }
  ...
  return (
    <div className={s.page}>
      <div className={s.headerRow}>
        ...
        <h1 className={s.title}>Canais de Vendas</h1>
      </div>
      <p className={s.subtitle}>Desempenho de cada canal: loja física, e-commerce, marketplaces.</p>
      ...
      {data && !loading && (
        <>
          <div className={s.summaryGrid}>
            <div className={s.summaryCard}><span className={s.summaryValue}>{data.summary.totalChannels}</span><span className={s.summaryLabel}>Canais</span></div>
          ...
          <div className={s.tableWrap}>
            <table className={s.table}>
              <thead><tr><th>Canal</th><th>Vendas</th><th>Total</th><th>Participação</th></tr></thead>
              <tbody>
                {data.rows.length === 0
...
export default CanaisVendasReport;
