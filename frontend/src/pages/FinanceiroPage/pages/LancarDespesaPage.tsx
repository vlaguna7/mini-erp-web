...
const expenseCategories = [
  ...
  'Impostos', 'Fornecedores', 'Marketing', 'Manutenção', 'Transporte',
...
const LancarDespesaPage: React.FC = () => {
  ...
  return (
    <div className={s.page}>
      <div className={s.headerRow}>
        ...
        <h1 className={s.title}>Lançar Saída (Despesa)</h1>
      </div>
      <p className={s.subtitle}>Registre e acompanhe todas as saídas de caixa.</p>
      ...
      {!loading && (
        <>
          <div className={s.summaryGrid}>
            <div className={s.summaryCard}><span className={s.summaryValue}>{items.length}</span><span className={s.summaryLabel}>Despesas</span></div>
            <div className={s.summaryCard}><span className={s.summaryValue} style={{ color: '#dc2626' }}>{fmt(total)}</span><span className={s.summaryLabel}>Total de Saídas</span></div>
          </div>
          ...
          <div className={s.tableWrap}>
            <table className={s.table}>
              ...
              <tbody>
                {items.length === 0
                  ? <tr><td colSpan={7} className={s.emptyState}>Nenhuma despesa encontrada.</td></tr>
                  : items.map(item => (
                    <tr key={item.id}>
                      <td>{new Date(item.date).toLocaleDateString('pt-BR')}</td>
                      <td>{item.description}</td>
                      <td>{item.category || '—'}</td>
                      <td className={s.mono} style={{ color: '#dc2626' }}>{fmt(Number(item.value))}</td>
...
export default LancarDespesaPage;
