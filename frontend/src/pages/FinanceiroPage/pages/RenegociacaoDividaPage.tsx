...
const paymentMethods = [
  'Dinheiro', 'PIX', 'Cartão de Crédito', 'Boleto',
  'Transferência', 'Parcelamento', 'Cheque',
];

const RenegociacaoDividaPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ description: '', category: '', value: '', date: new Date().toISOString().slice(0, 10), paymentMethod: '', status: 'PENDENTE', notes: '' });
      setShowForm(false);
      load();
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Deseja remover esta renegociação?')) return;
    try { await financialService.deleteTransaction(id); load(); } catch { /* */ }
  };

  const total = items.reduce((acc, i) => acc + Number(i.value), 0);
  const pendente = items.filter(i => i.status === 'PENDENTE').reduce((acc, i) => acc + Number(i.value), 0);
  const pago = items.filter(i => i.status === 'PAGO').reduce((acc, i) => acc + Number(i.value), 0);

  return (
    <div className={s.page}>
      <div className={s.headerRow}>
        <button className={s.backBtn} onClick={() => nav('/financeiro')}><ArrowLeft size={16} /> Voltar</button>
        <h1 className={s.title}>Renegociação de Dívida</h1>
      </div>
      <p className={s.subtitle}>Registre acordos de renegociação com novos prazos e condições.</p>
      ...
      <div className={s.filterBar}>
        ...
        <button className={s.filterBtn} onClick={() => setShowForm(!showForm)} style={{ background: '#059669' }}>
          <Plus size={16} /> Nova Renegociação
        </button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: '1.25rem', marginBottom: '1.5rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', alignItems: 'end' }}>
          <div className={s.filterGroup}>
            <label className={s.filterLabel}>Descrição / Credor *</label>
            <input className={s.filterInput} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="Ex: Renegociação Fornecedor X" required />
          </div>
          <div className={s.filterGroup}>
            <label className={s.filterLabel}>Tipo de dívida</label>
          ...
          <div className={s.filterGroup}>
            <label className={s.filterLabel}>Valor renegociado (R$) *</label>
          ...
          <div className={s.filterGroup}>
            <label className={s.filterLabel}>Observações / Condições</label>
            <input className={s.filterInput} value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} placeholder="Condições do acordo..." />
          </div>
      ...
      {!loading && (
        <>
          <div className={s.summaryGrid}>
            <div className={s.summaryCard}><span className={s.summaryValue}>{items.length}</span><span className={s.summaryLabel}>Renegociações</span></div>
            <div className={s.summaryCard}><span className={s.summaryValue}>{fmt(total)}</span><span className={s.summaryLabel}>Valor Total</span></div>
            <div className={s.summaryCard}><span className={s.summaryValue} style={{ color: '#eab308' }}>{fmt(pendente)}</span><span className={s.summaryLabel}>Pendente</span></div>
            <div className={s.summaryCard}><span className={s.summaryValue} style={{ color: '#059669' }}>{fmt(pago)}</span><span className={s.summaryLabel}>Quitado</span></div>
          </div>
            ...
            <table className={s.table}>
              <thead>
                <tr>
                  ...
                  <th>Pagamento</th><th>Status</th><th>Observações</th><th></th>
                </tr>
              </thead>
              <tbody>
                {items.length === 0
                  ? <tr><td colSpan={8} className={s.emptyState}>Nenhuma renegociação encontrada.</td></tr>
                  : items.map(item => (
                    <tr key={item.id}>
                      ...
                      <td>
                        <span className={`${s.badge} ${item.status === 'PAGO' ? s.badgeGreen : s.badgeYellow}`}>
                          {item.status === 'PAGO' ? 'Quitado' : 'Pendente'}
                        </span>
                      </td>
...
export default RenegociacaoDividaPage;
